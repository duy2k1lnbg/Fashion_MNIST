...
ten_layer_model = keras.Sequential([
    keras.layers.Flatten(input_shape=(28, 28)),
    keras.layers.Dense(128, activation=tf.nn.relu),
    keras.layers.Dense(128, activation=tf.nn.relu),
    keras.layers.Dense(128, activation=tf.nn.relu),
...
ten_layer_model.compile(optimizer='adam',
              loss='sparse_categorical_crossentropy',
              metrics=['accuracy'])
...
'''
verbose có 3 giá trị:
    0: không ghi bất cứ gì vào stdout
    1: ghi tiến trình dưới dạng thanh progress bar
    2: ghi mỗi epoch dưới dạng một dòng log
'''
...
'''
   three_layer_model.save('my_model.h5'): Lưu file vừa huấn luyên dưới dạng my_model.h5
'''
three_layer_model.save('my_model.h5')
